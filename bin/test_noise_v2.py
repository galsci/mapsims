...
def get_spectra(tube,omap,nside,res,white):
    zeros = []
    if nside is not None:
        mlmax = nside * 2
        alms =[ hp.map2alm(omap[i,0,...],lmax=mlmax,pol=True)  for i in range(2)]
    else:
        mlmax = 4096 * 2 / res
        alms =[ cs.map2alm(omap[i,0,...],lmax=mlmax,spin=[0,2])  for i in range(2)]
...
for key in config.keys():
    ...
    nsim = noise.SONoiseSimulator(nside=nside,shape=shape,wcs=wcs, homogenous = c.homogenous, sky_fraction = c.fsky)
    ...
    ls,c11,c22,c12,zeros = get_spectra(c.tube,omap,nside,res,c.wnoise)
