...
def _DATAURL(healpix, version):
    if healpix:
        return (
            f"https://portal.nersc.gov/project/cmb/so_mapsims_data/{version}/healpix/",
        )
    else:
        return (f"https://portal.nersc.gov/project/cmb/so_mapsims_data/{version}/car/",)


def _PREDEFINED_DATA_FOLDERS(healpix, version):
    if healpix:
        return [
            f"/global/project/projectdirs/cmb/www/so_mapsims_data/{version}/healpix",  # NERSC
            f"/scratch/r/rbond/msyriac/data/sobs/so_mapsims_data/{version}/healpix",  # SCINET/NIAGARA
        ]
    else:
        return [
            f"/global/project/projectdirs/cmb/www/so_mapsims_data/{version}/car",  # NERSC
